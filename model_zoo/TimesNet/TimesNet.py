# Reference from "TIMESNET: TEMPORAL 2D-VARIATION MODELING FOR GENERAL TIME SERIES ANALYSIS"
...
def FFT_for_Period(x, k=2):
    ...
    # exclude the zero-frequency (DC) component from period computation
    frequency_list[0] = 0
    # take top-k frequencies from indices 1..N-1 to avoid zero-period division
...
class Configs:
    def __init__(self):
        ...
        # TODO: flexible seq_lens: 1. Lazy init self.projection 2. Padding
        self.seq_len = 720          
        self.label_len = None       
        self.pred_len = 0   # Not necessary for classification 
        self.enc_in = 2
        self.c_out = None
        self.d_ff = 128           
        self.d_model = 32
...
class TimesNet(Model):
    def __init__(self, num_classes=16, seq_len=720, input_dim=2):
        configs = Configs()
...
if __name__ == "__main__":
    # x = torch.ones((8,721,2))
    # timesnet = TimesNet(seq_len=721)
    # x = x.to("cuda")
    # timesnet.to("cuda")                
    # print(timesnet(x).shape)
    x = torch.ones((1,2048,2))
    model = TimesNet(seq_len=2048, num_classes=16, input_dim=2)
    print("-----------------thop-------------------")
